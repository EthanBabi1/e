  ...
  return (
    <section className="max-w-5xl mx-auto px-6 py-12">
      <div className="flex items-center gap-2 mb-4">
        ...
        <p className="label-small">{zones.length} zone{zones.length === 1 ? "" : "s"} open</p>
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {zones.map((zone) => (
          <div key={zone.zoneName} className="rounded-xl border border-mist p-4">
            <p className="text-xs text-graphite mb-1 capitalize">{zone.tier}</p>
            <p className="font-medium text-sm mb-2">{zone.zoneName}</p>
            {zone.priceUsd != null && <p className="font-display text-lg">${zone.priceUsd}</p>}
          </div>
        ))}
      </div>
    </section>
