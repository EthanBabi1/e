...
/**
 * Scroll-scrubbed frame-sequence hero. Pins via a tall section + inner
 * `sticky` div (rather than ScrollTrigger's own pin) so it composes
 * cleanly with this app's Lenis smooth-scroll provider. GSAP (not the
 * Framer Motion already used elsewhere in this project) specifically
 * for its scrollTrigger + timeline API, which suits this frame-by-frame
 * choreography better than Framer's viewport-triggered primitives —
 * both libraries coexist deliberately, this is the only place GSAP is
 * used.
 */
...
export function HeroScrub({
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      master.to(
        titleTopRef.current,
        { x: () => (window.innerWidth < 768 ? "-70vw" : "-60vw"), letterSpacing: "0.02em", ease: "power2.inOut", duration: 0.15 },
        0
      );
      master.to(
    ...
    >
      <div ref={stickyRef} className="sticky top-0 flex h-[100svh] w-full flex-col items-center justify-center overflow-hidden">
        ...
        <div
          ...
          style={{ background: "radial-gradient(ellipse at 50% 35%, rgba(255,255,255,0.08) 0%, rgba(0,0,0,0) 55%)" }}
        />
        <div
          aria-hidden
          className="absolute inset-0 z-0"
        ...
        <div className="relative z-10 flex h-full w-full flex-col items-center justify-center gap-3 md:gap-4">
          ...
          <div
            ...
            style={{ width: `min(96vw, calc(72svh * ${aspect}))`, height: `min(72svh, 96vw / ${aspect})`, aspectRatio: aspect }}
          >
            <div aria-hidden className="pointer-events-none absolute inset-0 z-20 shadow-[inset_0_0_120px_rgba(0,0,0,0.45)]" />
            {framesOk && <canvas ref={canvasRef} aria-hidden className="absolute inset-0 h-full w-full object-cover" />}
          </div>
