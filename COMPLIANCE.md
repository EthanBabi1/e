...
| Stripe Connect account belongs to guardian, never minor | `lib/stripe/connect.ts` — `getOrCreateConnectAccount()`'s `ownerType` union has no "minor" option; the only racer-id entry point resolves a minor to their guardian's id first | Built (Phase 5) |
...
| Guardian approves listing before public, and winner before funds move | `zone_listings.guardianApprovedAt` gates public visibility (checked in `lib/marketplace/getListings.ts` and `lib/profile/getProfileData.ts`); winner approval is `approveWinnerAsGuardian()` in `lib/sponsorships/escrow.ts`, the required first step in the escrow state machine before a decal photo or fund release can happen | Built (Phase 3/4 listing gate; Phase 5 winner-approval gate), verified by `tests/integration/escrow.test.ts` |
...
| Full retention of minor-involved messages, except on a guardian's own deletion request | Nothing else in the product deletes a message; `lib/accounts/deletion.ts`'s `purgeRacerData()` is the sole path that removes them, and only as part of the racer's own account being deleted | Built (Phase 6) |
| Deletion request from guardian executes immediately, not the 90-day hold | `lib/accounts/deletion.ts` | Built (Phase 6), verified by `tests/integration/account-deletion.test.ts` |
